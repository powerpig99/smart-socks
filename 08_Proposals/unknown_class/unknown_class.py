...
Usage:
    ...
    # Test rejection
    python unknown_class.py --test --model ./models/rejecting_classifier.pkl --test-data ../../03_Data/features/test_unknown.csv
"""
...
class ConfidenceThresholdClassifier:
    ...
    def __init__(self, base_model, threshold: float = 0.6):
        ...
        self.scaler = None  # Will be set if needed
    ...
    def predict_proba(self, X):
        ...
        # Unknown probability = max(0, threshold - max_prob) / normalization
...
class NoveltyDetector:
    ...
    def fit_novelty_detector(self, X_target: np.ndarray):
        ...
        Args:
            X_target: Feature matrix of target activities only (no jumping/running)
        """
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Array of predictions with 'unknown' for novel samples
        """
        X_scaled = self.scaler.transform(X)
        
        # Stage 1: Check if sample is "known" or "novel"
        # IsolationForest returns 1 for inliers (known), -1 for outliers (novel)
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray, 
        ...
        Returns metrics for both rejection accuracy and classification accuracy.
        """
        ...
        # Rejection metrics
        true_positives = np.sum(is_unknown_actual & is_unknown_pred)  # Correctly rejected
        false_positives = np.sum(~is_unknown_actual & is_unknown_pred)  # Incorrectly rejected
        false_negatives = np.sum(is_unknown_actual & ~is_unknown_pred)  # Missed rejection
        ...
        # Classification accuracy on accepted samples
        ...
        return {
            ...
            'unknown_rejected': np.sum(is_unknown_pred),
            'total_unknown': np.sum(is_unknown_actual)
        }
...
def train_with_rejection(features_path: Path, 
    ...
    # Train base classifier on target activities only
    ...
    if method == 'confidence':
        ...
        # Save
        rejector.save(output_dir / 'rejecting_classifier.pkl')
        ...
        # Save
        full_detector.save(output_dir / 'novelty_detector.pkl')
...
def evaluate_rejection(model_path: Path, test_data_path: Path, method: str):
    ...
    # Calculate unknown class metrics
...
def compare_methods(features_path: Path, output_dir: Path):
    ...
    is_unknown = y == unknown_activity
    y_simulated = y.copy()
    y_simulated[is_unknown] = 'unknown'
    
    results = []
    
    for method, params in [
        ('confidence_0.5', {'method': 'confidence', 'threshold': 0.5}),
        ('confidence_0.6', {'method': 'confidence', 'threshold': 0.6}),
        ('confidence_0.7', {'method': 'confidence', 'threshold': 0.7}),
        ('novelty_0.05', {'method': 'novelty', 'contamination': 0.05}),
        ('novelty_0.10', {'method': 'novelty', 'contamination': 0.10}),
    ]:
        # Train
        train_with_rejection(features_path, output_dir / method, **params)
        
        # Evaluate
        if params['method'] == 'confidence':
            model = ConfidenceThresholdClassifier.load(output_dir / method / 'rejecting_classifier.pkl')
        else:
            model = NoveltyDetector.load(output_dir / method / 'novelty_detector.pkl')
        
        predictions = model.predict(X)
        
        # Metrics
        unknown_actual = y_simulated == 'unknown'
        unknown_pred = predictions == 'unknown'
        ...
        acc_on_accepted = np.mean(predictions[accepted] == y_simulated[accepted]) if np.sum(accepted) > 0 else 0
